...
export async function POST(req) {
    try {
        const { title, coupon, buttonLink, image, order } = await req.json();
        ...
        const newBanner = new PromotinalBanner({ title, coupon, buttonLink, order: nextOrder, image });
...
export async function PATCH(req) {
    try {
        const { id, title, coupon, buttonLink, image, order } = await req.json();
        const updatedBanner = await PromotinalBanner.findByIdAndUpdate(id, { title, coupon, buttonLink, order, image }, { new: true });
