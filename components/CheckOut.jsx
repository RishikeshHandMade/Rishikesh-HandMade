...
//   const { cart: rawCart, setCart } = useCart();
...
//       } catch {
//         setCart([]); // default to empty array if parsing fails
//       }
//       localStorage.removeItem("checkoutCart");
//     } else {
//       setCart(rawCart); // default to context cart if no localStorage cart
//     }
//   }, []);
...
//       shippingCost: shippingOptions.find(opt => opt.value === shipping)?.cost || 0
//     };
//   };
...
//     e.preventDefault();
//     if (loading) return;
//     if (!firstName || !lastName || !country || !street || !city || !state || !zip || !phone || !email) {
        ...
        {checkoutData ? (
          <>
                ...
                <div key={item.id} className="flex items-center gap-3 py-3">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-center w-full">
                      <div>
                        <div className="font-medium text-md leading-tight">{item.name}</div>
                        <div className="text-xs text-gray-500">Base: ₹{item.originalPrice}</div>
                        {item.discountPercent || item.discountAmount ? (
                          <div className="text-xs text-green-700">Discount: {item.discountPercent ? `${item.discountPercent}%` : `₹${item.discountAmount}`}</div>
                        ) : null}
                        <div className="text-xs text-gray-500">CGST: {item.cgst || 0}% | SGST: {item.sgst || 0}%</div>
                      </div>
                      <div className="text-md text-black font-semibold text-right min-w-[110px]">
                        ₹{item.afterDiscount?.toFixed(2) ?? item.price?.toFixed(2)} x {item.qty}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex justify-between items-center text-sm mb-1">
              ...
              <span>-₹{checkoutData.totalDiscount?.toFixed(2)}</span>
            </div>
            {checkoutData.promo && (
              <div className="flex justify-between items-center text-sm mb-1 text-green-700">
                <span>Promo Code ({checkoutData.promo.code})</span>
                <span>-₹{checkoutData.promo.discount?.toFixed(2)}</span>
              </div>
            )}
            <div className="flex justify-between items-center text-sm mb-1">
              <span>Tax Total</span>
