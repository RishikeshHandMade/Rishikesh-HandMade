...
function EyeIcon() {
  return (
    <svg width="20" height="20" fill="none" stroke="#222" strokeWidth="2" viewBox="0 0 24 24">
      ...
      <path d="M2 12C4.5 7 12 7 12 7s7.5 0 10 5c-2.5 5-10 5-10 5s-7.5 0-10-5z" />
    </svg>
  );
}
...
const OnlineOrderLog = () => {
  ...
  const fetchOrders = async (page = 1) => {
    try {
      setLoading(true);
      const response = await fetch(`/api/orders/admin?type=online&page=${page}&limit=${pagination.limit}`);
            ...
            <div className="flex gap-2">
              ...
              {(filters.orderId || filters.startDate || filters.endDate) && (
                <button
                  ...
                  className="px-3 py-2 bg-red-500 hover:bg-gray-300 rounded-md text-sm"
                >
                  Reset
                </button>
              )}
              ...
              {filteredOrders.length > 0 ? (
                filteredOrders.map((order) => (
                  <tr key={order._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm">{order.transactionId}</td>
                    <td className="px-4 py-3 text-sm">
                      <div>
                ...
                <tr>
                  <td colSpan={columns.length} className="px-4 py-6 text-center text-gray-500">
                    {orders.length === 0 ? 'No online orders found' : 'No orders match your filters'}
                  </td>
                </tr>
              )}
            </tbody>
