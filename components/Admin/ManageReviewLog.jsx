"use client";
import React from "react";

const beige = "#f8f6f1";
const border = "1px solid #222";

const filterButtons = [
  { label: "Delete", color: "#fff", bg: "#e57373" },
  { label: "Inactive", color: "#222", bg: "#ffe082" },
  { label: "Active", color: "#fff", bg: "#81c784" },
];

const columns = [
  "Date",
  "Review",
  "Name",
  "Action",
  "View",
];

function SearchIcon() {
  return (
    <svg width="18" height="18" fill="none" stroke="#222" strokeWidth="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="7"/><path d="M16.65 16.65L21 21"/></svg>
  );
}

function DropdownIcon() {
  return (
    <svg width="16" height="16" fill="none" stroke="#222" strokeWidth="2" viewBox="0 0 24 24"><path d="M6 9l6 6 6-6"/></svg>
  );
}

function EyeIcon() {
  return (
    <svg width="20" height="20" fill="none" stroke="#222" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><path d="M2 12C4.5 7 12 7 12 7s7.5 0 10 5c-2.5 5-10 5-10 5s-7.5 0-10-5z"/></svg>
  );
}

const ManageReviewLog = () => {
  return (
    <div className="reviewlog-root">
      {/* Header and Controls */}
      <div className="reviewlog-header-row">
        <div className="reviewlog-title-col">
          <h2>Manage Reviews Log</h2>
          <div className="reviewlog-wide-searches">
            <div className="reviewlog-wide-search">
              <input placeholder="Approved Review" />
              <button className="icon-btn" aria-label="search">
                <SearchIcon />
              </button>
            </div>
            <div className="reviewlog-wide-search">
              <input placeholder="Reject Review" />
              <button className="icon-btn" aria-label="search">
                <SearchIcon />
              </button>
            </div>
          </div>
        </div>
        <div className="reviewlog-controls-col">
          <div className="reviewlog-searchbar">
            <input placeholder="Search By" />
            <button className="icon-btn" aria-label="search">
              <SearchIcon />
            </button>
          </div>
          <div className="reviewlog-filters">
            {filterButtons.map((btn) => (
              <button
                key={btn.label}
                className="filter-btn"
                style={{ background: btn.bg, color: btn.color }}
              >
                {btn.label}
              </button>
            ))}
          </div>
        </div>
      </div>

      {/* Table */}
      <div className="reviewlog-table-wrap">
        <table className="reviewlog-table">
          <thead>
            <tr>
              {columns.map((col) => (
                <th key={col}>{col}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {[...Array(5)].map((_, i) => (
              <tr key={i}>
                {/* Date */}
                <td></td>
                {/* Review */}
                <td></td>
                {/* Name */}
                <td></td>
                {/* Action (dropdown) */}
                <td>
                  <button className="icon-btn">
                    <DropdownIcon />
                  </button>
                </td>
                {/* View */}
                <td>
                  <button className="icon-btn">
                    <EyeIcon />
                  </button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>

      {/* Pagination */}
      <div className="reviewlog-pagination-row">
        <div className="pagination">
          <button className="icon-btn" aria-label="Prev">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#222" strokeWidth="2"><path d="M15 18l-6-6 6-6"/></svg>
          </button>
          {[1, 2, 3, 4, 5].map((num) => (
            <button
              key={num}
              className={`page-btn${num === 1 ? " active" : ""}`}
            >
              {num}
            </button>
          ))}
          <button className="icon-btn" aria-label="Next">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#222" strokeWidth="2"><path d="M9 6l6 6-6 6"/></svg>
          </button>
        </div>
        <div className="showing-label">Showing 50 Data</div>
      </div>

      <style jsx>{`
        .reviewlog-root {
          background: ${beige};
          padding: 32px 16px 24px 16px;
          min-height: 100vh;
          font-family: 'Inter', Arial, sans-serif;
        }
        .reviewlog-header-row {
          display: flex;
          align-items: flex-start;
          justify-content: space-between;
          flex-wrap: wrap;
          gap: 18px;
          margin-bottom: 34px;
        }
        .reviewlog-title-col {
          flex: 2 1 340px;
          min-width: 260px;
        }
        .reviewlog-title-col h2 {
          margin: 0 0 22px 0;
          font-size: 2rem;
          font-weight: 700;
          text-align: left;
        }
        .reviewlog-wide-searches {
          display: flex;
          flex-direction: column;
          gap: 16px;
        }
        .reviewlog-wide-search {
          display: flex;
          align-items: center;
          background: #fff;
          border-radius: 8px;
          border: ${border};
          box-shadow: 0 1px 2px rgba(0,0,0,0.03);
          padding: 2px 10px 2px 14px;
          max-width: 420px;
          height: 44px;
        }
        .reviewlog-wide-search input {
          border: none;
          outline: none;
          font-size: 1rem;
          background: transparent;
          flex: 1;
        }
        .reviewlog-controls-col {
          flex: 1 1 180px;
          min-width: 170px;
          display: flex;
          flex-direction: column;
          align-items: flex-end;
          gap: 12px;
        }
        .reviewlog-searchbar {
          display: flex;
          align-items: center;
          background: #fff;
          border-radius: 8px;
          border: ${border};
          box-shadow: 0 1px 2px rgba(0,0,0,0.03);
          padding: 2px 10px 2px 14px;
          width: 200px;
          height: 40px;
        }
        .reviewlog-searchbar input {
          border: none;
          outline: none;
          font-size: 1rem;
          background: transparent;
          flex: 1;
        }
        .reviewlog-filters {
          display: flex;
          flex-direction: column;
          gap: 10px;
          width: 100%;
        }
        .filter-btn {
          border: none;
          border-radius: 8px;
          font-size: 1rem;
          font-weight: 500;
          padding: 7px 0;
          margin: 0;
          cursor: pointer;
          box-shadow: 0 1px 2px rgba(0,0,0,0.04);
          transition: filter 0.15s;
        }
        .filter-btn:hover {
          filter: brightness(0.95);
        }
        .reviewlog-table-wrap {
          overflow-x: auto;
          background: #fff;
          border-radius: 12px;
          box-shadow: 0 2px 8px rgba(0,0,0,0.05);
        }
        .reviewlog-table {
          width: 100%;
          border-collapse: separate;
          border-spacing: 0;
          background: ${beige};
        }
        .reviewlog-table th, .reviewlog-table td {
          border: ${border};
          padding: 12px 10px;
          text-align: left;
          background: ${beige};
          font-size: 1rem;
        }
        .reviewlog-table th {
          font-weight: 700;
          background: #f3ede2;
        }
        .reviewlog-table td {
          min-width: 80px;
        }
        .icon-btn {
          background: none;
          border: none;
          padding: 4px;
          cursor: pointer;
          display: flex;
          align-items: center;
          justify-content: center;
        }
        .reviewlog-pagination-row {
          display: flex;
          align-items: center;
          justify-content: flex-start;
          gap: 24px;
          margin-top: 28px;
          flex-wrap: wrap;
        }
        .pagination {
          display: flex;
          align-items: center;
          gap: 6px;
        }
        .page-btn {
          background: #fff;
          border: ${border};
          border-radius: 6px;
          width: 32px;
          height: 32px;
          font-size: 1rem;
          font-weight: 500;
          cursor: pointer;
          transition: background 0.14s, color 0.14s;
        }
        .page-btn.active {
          background: #222;
          color: #fff;
        }
        .showing-label {
          margin-left: auto;
          font-size: 1rem;
          color: #444;
          font-weight: 500;
        }
        @media (max-width: 900px) {
          .reviewlog-header-row {
            flex-direction: column;
            align-items: stretch;
            gap: 18px;
          }
          .reviewlog-controls-col {
            flex-direction: row;
            align-items: flex-start;
            gap: 18px;
          }
          .reviewlog-filters {
            flex-direction: row;
            gap: 10px;
            width: auto;
          }
        }
        @media (max-width: 600px) {
          .reviewlog-table th, .reviewlog-table td {
            font-size: 0.92rem;
            padding: 8px 6px;
          }
          .reviewlog-title-col h2 {
            font-size: 1.3rem;
          }
          .reviewlog-wide-search,
          .reviewlog-searchbar {
            max-width: 100%;
            min-width: 120px;
          }
        }
      `}</style>
    </div>
  );
};

export default ManageReviewLog;
